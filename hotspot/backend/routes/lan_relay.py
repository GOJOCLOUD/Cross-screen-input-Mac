...
# -*- coding: utf-8 -*-
"""
局域网中转站路由（内置 FastAPI 版本）

职责：
- 在局域网模式下，作为 PC / Phone 之间的 WebSocket 中转站
- 提供简单的管理界面和健康检查接口

注意：
- 该模块由主应用 main.py 挂载，最终路径形如：
  - 管理界面:        /lan/
  - 健康检查:        /lan/health
  - 状态查询:        /lan/api/status
  - WebSocket 入口: /lan/ws
...
<body>
    ...
    <script>
        async function loadStatus() {
            try {
                const res = await fetch('/lan/api/status');
...
async def health() -> dict:
    """简单健康检查，便于排查问题"""
    status = _get_status()
    return {"status": "ok", "pcs": status["pc_count"], "phones": status["phone_count"]}
...
# 注意：异常处理需要在 main.py 中的 app 级别注册
# 这里保留处理函数供 main.py 使用
async def http_exception_handler(request: Request, exc: StarletteHTTPException):
    """404 时重定向到管理界面，避免显示 detail not found"""
    if exc.status_code == 404:
        # 注意：路由挂载在 /lan 前缀下，这里的相对路径会由 FastAPI 处理
        return RedirectResponse(url="/lan/", status_code=302)
